...
class CheckoutController extends Controller
{
	...
	{		
		if($locationId > 0) {
			...
			return view('appointments.refundInvoice', compact('locationId','clientId','staffLists','itemArr','serviceTaxes','productTaxes','taxFormula','paymentTypes','invoiceId','staffTip','ClientInfo'));
		}
	}
	...
	public function addItemToCheckout(Request $request)
	{
		if($request->ajax())
		{
								...
								<div class="row px-8">
									<div class="col-md-1 col-sm-6">
										<div class="form-group">
											...
											<input class="form-control qtinpt'.$uniqid.'" readonly value="1" name="quantity[]" type="text">
										</div>
									</div>
			...
			{
				$productData = InventoryProducts::select('id','product_name','initial_stock','retail_price','special_rate')->where('id', $id)->where('user_id', $AdminId)->first();
				...
				$producthtml = '<div class="card productItm cardId'.$uniqid.'" data-id="'.$uniqid.'">
							<div class="card-body border-left border-primary border-3">
								...
								$producthtml .= '
									<input type="hidden" name="item_id[]" value="'.$productData->id.'">
								...
								<div class="row px-8">
									<div class="col-md-1 col-sm-6">
										<div class="form-group">
											<label class="font-weight-bolder">Quantity</label>
											<input class="form-control qty_inpt qtinpt'.$uniqid.'" value="1" data-uid="'.$uniqid.'" name="quantity[]" type="text">
										</div>
									</div>
									<div class="col-md-3 col-sm-6" id="pricing-type">
										<div class="form-group">
							...
							<div class="card-body border-left border-primary border-3">
								...
								<div class="row px-8">
									<div class="col-md-1 col-sm-6">
										<div class="form-group">
											<label class="font-weight-bolder">Quantity</label>
											<input class="form-control qty_inpt qtinpt'.$uniqid.'" readonly value="1" data-uid="'.$uniqid.'" name="quantity[]" type="text">
										</div>
									</div>
									<div class="col-md-3 col-sm-6" id="pricing-type">
										<div class="form-group">
				...
				if($remove_tip == 0) {
					...
					if(!empty($staff_tips)) {
						...
						if(!empty($client) && !empty($$remidernoti)){
							...
							$SendMail = Mail::to($TO_EMAIL)->cc([$CC_EMAIL])->send(new forTippingNotification($FROM_EMAIL,$FROM_NAME,$SUBJECT,$AppointmentServices,$Appointment,$UniqueId,$client,$service,$remidernoti));	
