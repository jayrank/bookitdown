    ...
    public function index()
    {
		if (Auth::user()->can('home')) 
		{
			...
			for ($i=0; $i <7 ; $i++) {
				...
				$currentWeekdaytotal[$i]['appointment'] = AppointmentServices::select(DB::raw('sum(special_price) as sum_special_price'))->whereDate('appointment_date', '=', $currentWeekdaytotal[$i]['weekdate'])->get('appointment_total'); //->where('appointment_services.user_id', $AdminId)
			}
			//
			...
			$upcomingAppointmentsTemp = Appointments::where('user_id',$AdminId)
									...
									->select('appointment_date',DB::raw('sum(appointments.is_cancelled = 1) as confirmed_appointments'),DB::raw('sum(appointments.is_cancelled = 0) as cancelled_appointments'))
									->get()
									->toArray();
