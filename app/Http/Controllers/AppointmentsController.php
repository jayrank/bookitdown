...
class AppointmentsController extends Controller
{
    ...
    public function createAppointment($locationId = null)
    {
		...
		// Get all staff
		$Staff = Staff::select('staff.staff_user_id','users.first_name','users.last_name')->where(['user_id'=>$AdminId])->where(['is_appointment_booked'=>1])->join('users','users.id','=','staff.staff_user_id')->orderBy('staff.id', 'ASC')->get()->toArray();
		...
		if(empty($_GET['dateTime'])) {
			...
			$NearestTime = date('H:i:00', strtotime($_GET['dateTime']));
		}
				...
				if(!empty($Clients)) {
					foreach($Clients as $Clientdata){
						...
						<div class="d-flex align-items-center border-bottom p-6 customer-data" onclick="getClientHistory('.$Clientdata['id'].');">
							...
							<div>
								<h6 class="font-weight-bolder sidebar-client-name">'.$Clientdata['firstname'].' '.$Clientdata['lastname'].'</h6>
	...
	public function getClientInformation(Request $request){
        ...
        if ($request->ajax())
        {
						...
						<div class="d-flex align-items-center flex-grow-1">
							...
							<div class="d-flex flex-wrap align-items-center justify-content-between w-100">
								<div class="d-flex flex-column align-items-cente py-2 w-75">
									...
									<span class="text-muted font-weight-bold">'.$duration.' with  '.$staff_name.' </span>
								</div>
								<h6 class="font-weight-bolder py-4">CA $'.$special_price.'</h6>
							</div>
						</div>
					</div>';//<i class="fa fa-heart text-danger"></i>
				}
			}
			...
			$HTML = '
			<div class="view-appoinment mb-2" style="height: calc(100vh - 165px);" style="display:none;">							
				<div id="" class="bg-white active">
					...
					<div class="card-body p-1">
						...
						<div class="tab-content mt-5 my-scrollbar" id="myTabContent" style="height: calc(100vh - 380px);overflow-y: scroll;overflow-x: hidden;">
	...
	public function viewInvoice($invoiceId = null)
	{
		...
		if($invoiceId != '')
		{
			...
			$checkInvRefund = Invoice::select('id')->where(['original_invoice_id'=>$invID,'invoice_status'=>'2'])->where('user_id', $AdminId)->first();

			//testQuery
			$checkInvRefund2 = Invoice::select('id')->where(['original_invoice_id' => $invID,'invoice_status' => '3'])->where('user_id', $AdminId)->first();


			//test
			$isVoided = 0;
			if(!empty($checkInvRefund2)) 
			{
				$isVoided = 1;	
				
			}	
// dd($isVoided);
			// dd($checkInvRefund->invoice_status);
			...
			$InvoiceVoucher = InvoiceVoucher::select('*')->where('invoice_id', $invID)->get()->toArray();
			...
			if($Invoice['client_id'] != 0)
			{
				...
				if(!empty($clientInvoices)) {
					...
					$ClientInovices .= '<h3>No invoice</h3>';
				}
			}
			
			return view('appointments.viewInvoice',compact('invoiceId','Invoice','InvoiceItemsInfo','ClientInfo','InvoiceTaxes','TotalStaffTip','LocationInfo','PaymentDoneBy','VoucherSold','PlanSold','InvoiceTemplate','isRefunded','PreviousAppointment','PreviousServices','TotalSpend','soldProduct','ClientProducts','clientInvoices','ClientInovices','InvoiceVoucher','isVoided'));
		}
	}
	...
	public function markInvoiceVoid(Request $request){
		if ($request->ajax())
		{
			...
			if(!empty($Invoice)) {
				$invoiceStatus = Invoice::select('invoice_status')->where('id',$Invoice['id'])->where('user_id', $AdminId)->orderBy('id', 'desc')->get()->first()->toArray();
				if($invoiceStatus == 2){
					$Success = 2;
				}else{
					if(!empty($InvoiceItems)) {
						foreach($InvoiceItems as $InvoiceItemData) {
							...
							if($InvoiceItemData['item_type'] == 'services'){
								...
								{
									$newStock1 = $InvoiceStock * $InvoicePrice;
			...
			if($Success == 1){
				...
				$data["status"] = true;
				$data["message"] = "Refunded invoice cannot be voided!";	
			} else {
				$data["status"] = true;
				$data["message"] = "Something went wrong!";		
			}
