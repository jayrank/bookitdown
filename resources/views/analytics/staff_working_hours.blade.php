...
@section('content')
                                    ...
                                    <div class="card-custom card-stretch gutter-b">
                                        ...
                                        <div class="content-header" style="margin-top:2%">
                                            <div class="mb-3">
                                                ...
                                                <h3 class="my-3 font-weight-bolder">Staff working hours</h3>
                                            </div>
                                                    ...
                                                    <div class="dropdown dropdown-inline mr-2">
                                                        ...
                                                        <div class="dropdown-menu dropdown-menu-sm dropdown-menu-right">
                                                            ...
                                                            <ul class="navi flex-column navi-hover py-2">
                                                                <li class="navi-item getStaffWorkingHoursExcel">
                                                                ...
                                                                <li class="navi-item getStaffWorkingHoursCSV">
                                                                    <a href="#" class="navi-link">
                                                                        <span class="navi-icon">
                                                                ...
                                                                <li class="navi-item getStaffWorkingHoursPDF">
                                                                    <a href="#" class="navi-link">
                                                                        <span class="navi-icon">
                                            ...
                                            <div class="p-4 selectedFilters" style="border:1px solid #CCC">
                                                Selected filtes: <span class="bagde p-2 rounded badge-primary">
                                                    Location is {{ $Locations[0]['location_name'] }}</span>
                                                    <!-- <span class="bagde p-2 rounded badge-primary">
                                                    Staff is All Staff</span> -->
                                            </div>
                                        </div>
                                        @foreach($Staff as $StaffKey => $StaffValue)
                                            <div class="row reports mx-1 my-3 staffTable" id="staffTable{{$StaffValue['staff_user_id']}}">
                                                <h3 class="my-4">{{ $StaffValue['first_name'] }} {{ $StaffValue['last_name'] }}</h3>
                                                <div class="table-responsive bg-white">
                                                    <table cellspacing="10" cellpadding="10" class="table getStaffWorkingHoursTable{{$StaffValue['staff_user_id']}}">
                                                        <thead>
                                                            <tr>
                                                        ...
                                                        <tfoot>
                                                            <tr class="font-weight-bolder">
                                                                ...
                                                                <td colspan="2"></td>
                                                                <td class="getStaffWorkingHoursTableFooter{{$StaffValue['staff_user_id']}}" style="font-weight: bold;"></td>
                                                            </tr>
                                                        </tfoot>
                                                    </table>
                                                </div>
    ...
    <script>
        jQuery(document).ready(function () {
            ...
            $('.ApplyFilterButton').on('click', function(){
                ...
                var temp = 'Selected filtes: <span class="bagde p-2 rounded badge-primary"> Location is '+ $("#userLocations option:selected").text() +'</span>';
                
                var isStaff = $("#staffUserID option:selected").val();
                if(isStaff){
                    temp += '<span class="bagde p-2 rounded badge-primary" style="margin-left:5px;"> Staff is '+ $("#staffUserID option:selected").text() +'</span>';
                }
                $(".selectedFilters").val('');
                $(".selectedFilters").html(temp);


                $(".staffTable").hide();
                // $(".selectedFilters").append($("#staffUserID").val())
                getStaffWorkingHours();
                closeNav();
            });
            ...
            $(document).on('click','.getStaffWorkingHoursPDF', function(){
                KTApp.blockPage();
                ...
                var staffUserIDArray = [];

                if(!staff_id){
                    $("#staffUserID option").each(function(key, value){
                        // console.log(value.value);
                        staffUserIDArray.push(value.value);
                        // $("#staffTable"+value.value).show();
                    });
                }else{
                    staffUserIDArray.push(staff_id);
                    // $("#staffTable"+staff_id).show();
                }
                var filtered = staffUserIDArray.filter(function (el) {
                    return (el != null) && (el != "");
                });

                staffUserIDArray = filtered;

                if (date != '') {
                    var fileName = 'staff_working_hours_' + start_date + '_' + end_date + '.pdf';
                } else {
                    var fileName = 'staff_working_hours.pdf';
                }

                $.ajax({
                    url: '{{ route("getStaffWorkingHoursPDF") }}',
                    type: 'POST',
                    data: {
                        start_date: start_date,
                        end_date: end_date,
                        location_id: location_id,
                        staff_id: staffUserIDArray
                    },
                    xhrFields: {
                        responseType: 'blob'
                    },
            ...
            $(document).on('click','.getStaffWorkingHoursCSV', function(){
                KTApp.blockPage();
                var date = $(".kt_date").val();
                ...
                if (date != '') {
                    var fileName = 'staff_working_hours_' + start_date + '_' + end_date + '.csv';
                } else {
                    var fileName = 'staff_working_hours.csv';
                }

                $.ajax({
                    url: '{{ route("getStaffWorkingHoursCSV") }}',
                    type: 'POST',
                    data: {
                        start_date: start_date,
                        end_date: end_date,
            ...
            $(document).on('click','.getStaffWorkingHoursExcel', function(){
                KTApp.blockPage();
                var date = $(".kt_date").val();
                ...
                if (date != '') {
                    var fileName = 'staff_working_hours_' + start_date + '_' + end_date + '.xlsx';
                } else {
                    var fileName = 'staff_working_hours.xlsx';
                }

                $.ajax({
                    url: '{{ route("getStaffWorkingHoursExcel") }}',
                    type: 'POST',
                    data: {
                        start_date: start_date,
                        end_date: end_date,
        ...
        function getStaffWorkingHours(){
            ...
            $.each(staffUserIDArray, function(staffUserIDArrayKey, staffUserIDArrayValue){
                var datatable = $('.getStaffWorkingHoursTable'+ staffUserIDArrayValue).DataTable({
                    processing: true,
                    serverSide: true,
                    destroy: true,
                    "searching": false,
                    ...
                    "ordering": false,
                    ajax : {
                        url: '{{ route("getStaffWorkingHoursReport") }}',
                        type: 'POST',
                        dataType: 'JSON',
                        data: {
                            start_date: start_date,
                            end_date: end_date,
                            location_id: location_id,
                            staff_id: staffUserIDArrayValue
                        }
                    },
                    "drawCallback":function(settings){
                        ...
                        if(!json.isStaff){
                            $("#staffTable"+staffUserIDArrayValue).hide();
                        }else{
                            $("#staffTable"+staffUserIDArrayValue).show();
                        }

                        $('.getStaffWorkingHoursTableFooter'+ staffUserIDArrayValue).text(json.total);
                    },
                    columns: [
                        {data: 'date', profile: 'date'},
    ...
    <script type="text/javascript">
        ...
        function clearNav(){
            ...
            var temp = 'Selected filtes: <span class="bagde p-2 rounded badge-primary"> Location is '+ $("#userLocations option:selected").text() +'</span>';
            // temp += '<span class="bagde p-2 rounded badge-primary" style="margin-left:4px;">'+ $("#staffUserID option:selected").text() +'</span>';
            $(".selectedFilters").val('');
            $(".selectedFilters").html(temp);
