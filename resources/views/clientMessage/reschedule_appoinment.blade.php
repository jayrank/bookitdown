		...
		<div class="my-custom-body-wrapper">
			<div class="my-custom-header bg-white">
				<div class="p-4 d-flex justify-content-between align-items-center border-bottom">
					...
					<h1 class="font-weight-bolder">Reschedule Notification</h1>
					<div>
				...
				<div class="container-fluid bg-content">
					<div class="row">
						<div class="col-12 col-md-4 p-0 bg-content" style="height:calc(100vh - 80px);overflow-y:scroll">
							<form name="resChedule" id="resChedule">
							<div class="p-4">
								...
								<div class="form-group">
									<div class="checkbox-list mt-8">
										<label class="checkbox font-weight-bolder">
											<input @if(isset($resCh->is_rescheduleMessage) && $resCh->is_rescheduleMessage==1)checked @endif name="is_rescheduleMessage"
												type="checkbox">
											<span></span> Enable reschedule messages
										</label>
									</div>
								</div>
								<div class="form-group">
									<div class="checkbox-list mt-8">
										<label class="checkbox font-weight-bolder">
											<input @if(isset($resCh->is_displayServicePrice) && $resCh->is_displayServicePrice==1)checked @endif name="is_displayServicePrice" id="is_displayServicePrice"
												type="checkbox">
								...
								<div class="form-group">
									...
									<textarea rows="4" class="form-control" id="note" name="important_info"
										placeholder="e.g. Parking is available on the street">@if(isset($resCh->important_info) && $resCh->important_info!=null){{ $resCh->important_info }} @endif</textarea>
								</div>
							</div>
							</form>
						</div>
													...
													<div class="tab-content" id="myTabContent1">
														<div class="tab-pane fade active show" id="email"
															...
															<div class="card-body bg-dark text-white">
																<div class="message">
																	<h3>Hi {{ $client->firstname }} {{ $client->lastname }}, your appointment is rescheduled!</h3>
																	<span
																		class="mb-3 badge badge-pill badge-info">Confirmed</span>
																	<h4>Tuesday, 5 Jan at 10:00am</h4>
																	<div class="addr">
																		<h4>{{ $client->address }}</h4>
																		{{-- <p class="p-0 m-0 text-muted">Canada Way,
																			Burnby, British</p>
...
<script>
			...
			FormValidation
				...
				.on('core.form.valid', function() {
					...
					$.ajax({
						...
						data: {email : email, is_price_view : is_price_view, note : note, type : 3},		
	...
	$(document).on('click','#update', function(){
		...
		$.ajax({
			type: "POST",
			url: "{{ route('updateRescheduleNoti') }}",
			headers:{
						'X-CSRF-Token': '{{ csrf_token() }}',
					},
			data: form.serialize(),
