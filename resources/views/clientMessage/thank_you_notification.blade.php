		...
		<div class="my-custom-body-wrapper">
			<div class="my-custom-header bg-white">
				<div class="p-4 d-flex justify-content-between align-items-center border-bottom">
					...
					<h1 class="font-weight-bolder">Thank You Notification</h1>
					<button class="btn btn-primary" data-toggle="modal" data-target="#testMailModal" >Send test email</button>
					<button class="btn btn-lg btn-primary next-step" id="update">Save</button>
				</div>
			</div>
				...
				<div class="container-fluid bg-content">
					<div class="row">
						<div class="col-12 col-md-4 p-0 bg-content" style="height:calc(100vh - 80px);overflow-y:scroll">
							<form name="thankyounoti" id="thankyounoti">
							<div class="p-4">
								<h6 class="text-dark-50">
									This notification automatically sends to clients when their appointment start time
									is changed.
								</h6>
								<div class="form-group">
									<div class="checkbox-list mt-8">
										<label class="checkbox font-weight-bolder">
											<input @if(isset($notifi->is_thankyouMessage) && $notifi->is_thankyouMessage==1)checked @endif name="is_thankyouMessage"
												type="checkbox">
											<span></span> Enable thank you messages
										</label>
									</div>
								</div>
								<div class="form-group">
									<div class="checkbox-list mt-8">
										<label class="checkbox font-weight-bolder">
											<input @if(isset($notifi->is_displayServicePrice) && $notifi->is_displayServicePrice==1)checked @endif name="is_displayServicePrice" id="is_displayServicePrice"
												type="checkbox">
											<span></span> Display service prices and invoice link on email
										</label>
									</div>
								</div>
								...
								<div class="form-group">
									...
									<textarea rows="4" class="form-control" id="note" name="important_info"
										placeholder="e.g. Parking is available on the street">@if(isset($notifi->important_info) && $notifi->important_info!=null){{ $notifi->important_info }} @endif</textarea>
								</div>
							</div>
							</form>
						</div>
													...
													<div class="tab-content" id="myTabContent1">
														<div class="tab-pane fade active show" id="email"
															...
															<div class="card-body bg-dark text-white">
																<div class="message">
																	<h3>Tahnk you for visiting {{ $client->firstname }} {{ $client->lastname }}!</h3>
																	<span
																		class="mb-3 badge badge-pill badge-success">Completed</span>
																	...
																	<div
																		...
																		<div class="btn btn-info icon text-center">
																			<i class="text-white fas fa-star"></i>
																			Review
																		</div>
																		<div class="btn btn-success icon text-center">
																			<i
...
<script>
			...
			FormValidation
				...
				.on('core.form.valid', function() {
					...
					$.ajax({
						...
						data: {email : email, is_price_view : is_price_view, note : note, type : 4},		
	...
	$(document).on('click','#update', function(){
		var form = $("#thankyounoti");
		$.ajax({
			type: "POST",
			url: "{{ route('updatethankyouNotifi') }}",
			headers:{
						'X-CSRF-Token': '{{ csrf_token() }}',
					},
			data: form.serialize(),
