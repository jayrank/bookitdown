...
<div class="container-fluid p-0">
	<div class="my-custom-body-wrapper">
		...
		<form method="POST" id="saveConsultationForm" action="{{ route('updateconform') }}">
			@csrf
			
			<input type="hidden" name="country_code" id="country_code" value="1">
			<input type="hidden" name="total_section" id="total_section" value="0">
			<input type="hidden" name="updateform_id" id="form_id" value="{{ $formid }}">
						...
						<div class="row">
							<div class="col-12 col-md-2 p-0 border-right" style="height: calc(100vh - 90px);overflow-y: scroll;overflow-x: hidden;">
								<div class="">
									...
									<div class="card shadow-sm w-50 mx-auto mb-4 cursor-pointer" data-toggle="modal" data-target="#addClientDetailModal">
									...
									<!--div class="card shadow-sm w-50 mx-auto mb-4">
										<div class="card-body px-4 py-6">
											<span class="badge badge-info px-2 py-1 rounded-0" style="position: absolute;top:0;right:0"> soon
											</span>
											<div class="mx-auto" style="height: 32px;width: 32px;">
												<svg viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg">
													<path
									...
									<div class="card shadow-sm w-50 mx-auto mb-4 cursor-pointer" data-toggle="modal" data-target="#addCustomSectionModal">
										<div class="card-body px-4 py-6">
											...
											<p class="font-weight-bolder text-center m-0">Custom Section</p>
										</div>
									</div>
								</div>
							</div>
							...
							<div class="col-12 col-md-10 bg-content" style="height: calc(100vh - 90px);overflow-y: scroll;overflow-x: hidden;">
								...
								<div class="defaultView" @if(!empty($conFormClientDetails) || !empty($conFormCustomSection)) style="display:none;" @endif>
									<div class="card w-50 mx-auto my-6 checkClientSection" data-toggle="modal" data-target="#addNewSectionModal">
										<div class="rounded cursor-pointer" style="background-color: #e5f1ff;border: 1px solid #037aff !important;">
								...
								<div class="afterView" @if(empty($conFormClientDetails) && empty($conFormCustomSection)) style="display:block;" @else style="display:none;" @endif>
									...
									<div class="tab-content" id="myTabContent1">
										<div class="tab-pane fade active show" id="builder" role="tabpanel" aria-labelledby="builder-tab-1">
											...
											<div class="card w-70 mx-auto my-6 checkClientSection" data-toggle="modal" data-target="#addNewSectionModal">
						...
						<div class="row">
							<div class="col-12 col-md-4 border-right" style="height: calc(100vh - 90px);overflow-y: scroll;overflow-x: hidden;">
								...
								<div class="form-group">
									...
									<input type="text" class="form-control" id="consultation_form_name" name="consultation_form_name" value="{{ $ConsForm->name }}">
								</div>
								<hr>
								...
								<div class="form-group">
									...
									<select class="form-control" name="consultation_form_when">
										<option value="0" @if($ConsForm->complete==0) selected @endif>Every time they book an appointment </option>
										<option value="1" @if($ConsForm->complete==1) selected @endif>Only once</option>
									</select>
								</div>
								<div class="form-group" style="position: relative;">
									<label class="font-weight-bolder">Ask clients to complete when booking</label>
									<input type="text" id="serviceInput" class="form-control form-control-lg" readonly="" placeholder="All services" data-toggle="modal" data-target="#servicesModal" style="cursor: pointer;">
								...
								<div class="form-group mb-0">
									<div class="switch switch-md switch-icon switch-success" style="line-height: 28px;">
										<label class="d-flex align-item-center font-weight-bolder">
											<input type="checkbox" @if($ConsForm->signature==1) checked="checked" @endif name="consultation_form_is_signature">
		...
		<div class="modal-content">
			...
			<form id="addClientSectionForm" method="POST">
				...
				<div class="modal-body">
					<div class="form-group">
						<label class="font-weight-bolder">Section title</label>
						<input type="text" placeholder="add title" class="form-control" name="addClientSectionTitle" id="addClientSectionTitle" value="@if(isset($conFormClientDetails->section_title)) {{ $conFormClientDetails->section_title }} @endif">
					</div>
					<div class="form-group">
						<label class="font-weight-bolder">Section description <span class="text-muted">(Optional)</span></label>
						<input type="text" placeholder="add description" class=" form-control" name="addClientSectionDescription" id="addClientSectionDescription" value="@if(isset($conFormClientDetails->section_des)) {{ $conFormClientDetails->section_des }} @endif">
					</div>
					<hr>
						...
						<div class="row">
								...
								<div class="form-group">
									<div class="checkbox-list">
										<label class="checkbox font-weight-bolder">
											<input @if($conFormClientDetails->first_name==1)checked="" @endif id="clientSectionFirstName" name="clientDetailFields[]" type="checkbox">
							...
							<div class="col-12 col-sm-12 col-md-6">
								<div class="form-group">
									<div class="checkbox-list">
										<label class="checkbox font-weight-bolder">
											<input @if($conFormClientDetails->last_name==1)checked="" @endif id="clientSectionLastName" name="clientDetailFields[]" type="checkbox">
											<span></span> Last Name
										</label>
									</div>
								</div>
						...
						<div class="row">
							<div class="col-12 col-sm-12 col-md-6">
								<div class="form-group">
									<div class="checkbox-list">
										<label class="checkbox font-weight-bolder">
											<input @if($conFormClientDetails->email==1)checked="" @endif id="clientSectionEmail" name="clientDetailFields[]" type="checkbox">
											<span></span> Email
										</label>
									</div>
								</div>
							...
							<div class="col-12 col-sm-12 col-md-6">
								<div class="form-group">
									<div class="checkbox-list">
										<label class="checkbox font-weight-bolder">
											<input @if($conFormClientDetails->birthday==1)checked="" @endif id="clientSectionBirthday" name="clientDetailFields[]" type="checkbox">
											<span></span> Birthday
										</label>
									</div>
								</div>
						...
						<div class="row">
							<div class="col-12 col-sm-12 col-md-6">
								<div class="form-group">
									<div class="checkbox-list">
										<label class="checkbox font-weight-bolder">
											<input @if($conFormClientDetails->mobile==1)checked="" @endif id="clientSectionMobileNumber" name="clientDetailFields[]" type="checkbox">
							...
							<div class="col-12 col-sm-12 col-md-6">
								<div class="form-group">
									<div class="checkbox-list">
										<label class="checkbox font-weight-bolder">
											<input @if($conFormClientDetails->gender==1)checked="" @endif id="clientSectionGender" name="clientDetailFields[]" type="checkbox">
											<span></span> Gender
										</label>
									</div>
								</div>
						...
						<div class="row">
							<div class="col-12 col-sm-12 col-md-6">
								<div class="form-group">
									<div class="checkbox-list">
										<label class="checkbox font-weight-bolder">
											<input @if($conFormClientDetails->address==1)checked="" @endif id="clientSectionAddress" name="clientDetailFields[]" type="checkbox">
											<span></span> Address
										</label>
									</div>
								</div>
												...
												@foreach ($serviceData['service_price'] as $priceKey => $servicePrice)
													<li>
														<label for="all-{{ $serviceValue['category_title'] }}-{{ $serviceData['id'] }}-{{ $priceKey }}" class="checkbox">
															<input class="servicePriceIds" type="checkbox" name="value_checkbox[]" id="all-{{ $serviceValue['category_title'] }}-{{ $serviceData['id'] }}-{{ $priceKey }}" value="{{ $serviceData['id'] }}" @if(in_array($serviceData['id'],json_decode($ConsForm->service_id))) checked @endif>
															<span></span>
...
<script src="{{ asset('js/editConsultationForm.js') }}"></script>
<script>
	var alldetails = [];
	var ClientDetails = {!! json_encode($conFormClientDetails) !!};
	var CustomSection = {!! json_encode($conFormCustomSection) !!};
	var newarray = alldetails.concat(ClientDetails, CustomSection);
	@php 
		foreach ($conFormCustomSection as $item) {
			$showdata[] = $item;
		}
		$showdata[] = $conFormClientDetails;
		$sortdata = collect($showdata)
					->sortBy('section_id')
					->toArray();
		$groupdata = collect($showdata)
					->groupBy('section_id')
					->toArray();
	...
	var cou = {!! $total !!}
	var sortarray = {!! json_encode($sortdata) !!}
	var groupbyarray = {!! json_encode($groupdata) !!}
	var serviceId = {!! json_encode($ConsForm->service_id) !!}
