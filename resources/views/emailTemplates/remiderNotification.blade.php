						...
						<tbody>
										...
										<tbody>
											<tr>
												<td colspan="2" style="font-size:28px;padding:24px 0 8px 0;">
													Hi {{ $client->firstname }} {{ $client->lastname }} See You Soon
												</td>
												<span class="mb-3 badge badge-pill badge-danger">No-Show</span>
											</tr>
											<tr>
												<td colspan="2">
							...
							<tr>
								<td colspan="2" style="font-size:20px;line-height:29px;padding-top:24px;">
									<?php echo date("l, d F Y",strtotime($app->appointment_date)) ?> at
				...
				<td style="background-color:inherit;max-width:100%;width:100%;">
					<table width="100%" cellpadding="0" cellspacing="0"
						...
						<tbody>
										...
										<tbody>
											<tr>
												...
												<td style="font-size:16px;line-height:22px;text-align:right;">
													...
													$getappoint->total_amount }}@endif
												</td>
											</tr>
											<tr>
												<td style="font-size:14px;line-height:20px;opacity:0.5;">
													@php if($app->duration <= 0) {echo '00h 00min' ;} else {
														if(sprintf("%02d",floor($app->duration / 60)) > 0){echo
														sprintf("%02d",floor($app->duration / 60)).'h ';}
														if(sprintf("%02d",str_pad(($app->duration % 60), 2, "0",
														STR_PAD_LEFT)) > 0){echo sprintf("%02d",str_pad(($app->duration
							...
							@endif
							<tr>
								@if($remider->important_info!='')
								<div class=" d-flex justify-content-between ">
									<div>
										<h3 class="font-weight-bolder shownote">Important info</h3>
										<p class="m-0 shownote" id="desshow">{{ $remider->important_info }}</p>
									</div>
								</div>
								@endif
							</tr>
