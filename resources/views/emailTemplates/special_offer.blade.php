...
<body>
	<div class="border-light-dark card m-auto my-4 w-60"
		style="width: 100%;max-width: 550px;border: 1px solid #D1D3E0;border-radius: 0.42rem;margin: 0 auto;">
					...
					<tr>
						<td>
							<table class="main-table" cellpadding="0" cellspacing="0">
								<tbody>
									<tr>
										<td class="table-vertical-padding" colspan="3" style="padding-bottom: 40px;">
										</td>
									</tr>
									<tr>
		...
		<tr>
			...
			<td>
				<div class="offer-box-container"
					style="background:no-repeat url({{ url('public/uploads/schedule_library/offer-box-glow.png') }});background-color:#000000;background-size:100% auto;border-radius:8px;color:#FFFFFF;padding:10px;text-align:center;">
					<div class="offer-box"
						style="border:1px solid rgba(255, 255, 255, 0.4);border-radius:8px;padding:20px;">
						<div class="offer-box-headline"
							style="font-size:2rempx;font-weight:bold;margin-bottom:10px;text-transform:uppercase;">
							{{ ($message_data->discount_type == 1) ? 'CA $': '' }}
							{{ $message_data->discount_value }}
							{{ ($message_data->discount_type == 2) ? '%': '' }} off</div>
						<div class="offer-box-subheadline" style="font-size:15px;margin-bottom:25px;">your next {{
							($message_data->appointment_limit != "unlimited") ? $message_data->appointment_limit : '' }}
							appointment</div>
							...
							<tbody>
								<tr>
									...
									<td class="cta-button-cell"
										style="-webkit-text-size-adjust:none;background:#FFFFFF;border-radius:4px;font-size:14px;font-weight:bold;height:48px;line-height:48px;min-width:230px;text-align:center;text-decoration:none;text-transform:uppercase;">
										<a class="cta-button-link" clicktracking="off"
											href="{{ ($message_data->is_button == 2) ? $message_data->button_link : '#' }}"
											style="color:#000000 !important;display:inline-block;min-width:230px;padding:0 8px;text-decoration:none;">
				...
				@endphp
				<div class="offer-box-footer" style="color:#000000;font-size:14px;padding-top:20px;text-align:center;">
					Applies to {{ count($services) }} service{{ (count($services) > 1) ? 's':'' }} , valid for {{
					$message_data->valid_for }} days</div>
			</td>
			<td class=""></td>
		</tr>
