...
<div class="container-fluid p-0">
	<form id="smartCampaign" method="post">
		...
		<input type="hidden" name="minAmountType" id="minAmountType" value="0">

		<div class="my-custom-body-wrapper">
			<div class="my-custom-header">
				<div class="p-4 d-flex justify-content-between align-items-center border-bottom">
					<span class="d-flex">
						<p class="cursor-pointer m-0 px-6" onclick="window.location.href='{{ route('smart_campaigns') }}'"><i
						...
						<div class="col-12 col-md-4 p-0 last-hide" style="height:calc(100vh - 80px);overflow-y:scroll">
							<div class="p-4">
								<div class="edit-content-tab">
									<h6 class="my-3">Adjust the message text and upload your own images to customize the message
									...
									<div class="form-group">
										<label class="w-100 text-center">
											<div class="card-body" id="img-uploader" style="background-image: url(<?php echo url($defaultCampaign->image_path); ?>);background-size: 130px;background-repeat: no-repeat;background-position-x: 169px;">
												<a class="btn btn-white" data-target="#filemanager" data-toggle="modal">Change Header Image</a>
											</div>
											<input type="hidden" name="image_path" id="selectedImage" value="{{ $defaultCampaign->image_path }}">
										</label>
									</div>
									<h3 class="font-weight-bolder my-6">Text Content</h3>
									<div class="form-group">
										<label class="font-weight-bolder">Email subject</label>
										<input type="text" name="email_subject" id="email-subject" value="{{ $defaultCampaign->email_subject }}" class="form-control" placeholder="Enter email subject">
									...
									<div class="form-group">
										...
										<input type="text" name="headline_text" id="headline-input" value="{{ $defaultCampaign->headline_text }}" class="form-control" placeholder="">
									</div>
									<div class="form-group">
										<h6 class="font-weight-bolder">Body text</h6>
										<textarea id="body-input" name="body_text" class="form-control" placeholder="Body text here..." rows="8" class="text-dark-50 font-size-lg">{{ strip_tags($defaultCampaign->body_text) }}</textarea>
									</div>
								</div>
								<div class="edit-content-tab">
									...
									<div class="form-group d-flex align-items-center">
										<div>
											...
											<div class="input-group">
												<div class="input-group-prepend">
													<span class="input-group-text bg-white isPrice" id="selectedDiscount">
														%
													</span>
												</div>
												<input class="form-control numbers" id="discount-input" name="discount_value" value="10">
										...
										<div class="w-70 ml-4">
											<div class="tgl-radio-tabs">
												<input id="price" value="1" name="discount_type" type="radio" class="form-check-input tgl-radio-tab-child discountType" {{ ($defaultCampaign->discount_type == 1) ? 'checked' : '' }}>
												
												<label for="price" class="radio-inline">CA $</label>

												<input id="percent" value="2" type="radio" class="form-check-input tgl-radio-tab-child discountType" name="discount_type" {{ ($defaultCampaign->discount_type == 2) ? 'checked' : '' }}>
									...
									<div class="form-group">
										...
										<select class="form-control" onchange="appoinmentLimit(this)" name="appoinment_limit">
									...
									<div class="services">
										<h4 class="font-weight-bolder">Apply to <span class="servicesApply">0 services</span>
										</h4>
												...
												<span class="option-body text-dark">
													...
													@if($defaultCampaign->is_editable_client == 1)
														@if($defaultCampaign->edit_model_type == 1)
															<span id="selected_birthday_days">on the day of their birthday </span>
														@endif				
														<br>										
						...
						<div class="col-12 col-md-8 p-0 bg-white last-hide"
							...
							<div class="p-6">
								<div class="card w-70 m-auto">
									<div class="card-body p-12" id="smartCampaignPreview">
										...
										<div class="card-img" style="margin:40px auto;height:auto;width:100px">
											<img alt="img" id="img-preview" width="100%" height="auto" class="rounded"
												src="{{ url($defaultCampaign->image_path) }}" />
										</div>
										<h1 class="font-weight-bolder headline-text mb-10">{{ $defaultCampaign->headline_text }}</h1>
										<h6 class="text-dark-50 my-4 body-text">
											@php
												echo $defaultCampaign->body_text;
											@endphp
										</h6>
										<hr class="my-13">
										<div class="preview-card-discount">
											<div class="detail">
												<h1 class="my-4 font-weight-bolder text-uppercase">
													<span class="isPrice" style="display: {{ ($defaultCampaign->discount_type == '1') ? 'inline' : 'none' }};">$</span><span class="discount-text">{{ $defaultCampaign->discount_value }}</span><span class="isPercent" style="display: {{ ($defaultCampaign->discount_type == '2') ? 'inline' : 'none' }};">%</span> Off
												</h1>
												<p>your next <span class="appoinmentLimit">{{ $defaultCampaign->appoinment_limit }} appointment</span> </p>
												<div class="text-uppercase btn btn-light">
													Book Now
												</div>
											</div>
										</div>
										<p class="text-center mt-6 mb-0">Applies to <span class="servicesApply">0 service</span>, valid for <span class="validFor">{{ $defaultCampaign->valid_for }}</span> days</p>
									</div>
								</div>
							</div>
						</div>
						<div class="col-12 edit-content-tab last-tab">
							<div class="w-35 text-center center mx-auto my-30 align-items-center">
								...
								<div class="my-6 d-flex justify-content-around">
									<button type="button" class="btn btn-white" data-toggle="modal" data-target="#PreviewBirthdayModal">Preview</button>
														...
														@foreach ($serviceData['service_price'] as $priceKey => $servicePrice)
															<li>
																<label for="all-{{ $serviceValue['category_title'] }}-{{ $serviceData['id'] }}-{{ $priceKey }}" class="checkbox">
																	<input type="checkbox" name="service[]" id="all-{{ $serviceValue['category_title'] }}-{{ $serviceData['id'] }}-{{ $priceKey }}" value="{{ $serviceData['id'] }}">
																	<span></span>
					...
					<div class="modal-body">
						<div class="form-group">
							...
							<select class="form-control" name="daysBeforeBirthday" id="daysBeforeBirthday">
								<option value="0" data-text="on the day of their birthday">On the day of their birthday</option>
								<option value="3" data-text="3 days before their birthday">3 days before their birthday</option>
								<option value="7" data-text="a week before their birthday">One week before their birthday</option>
								<option value="14" data-text="two weeks before their birthday">Two weeks before their birthday</option>
							</select>
						</div>
					</div>
					<div class="modal-footer">
		...
		<div class="modal-content">
			...
			<div class="modal-body">
				<ul class="nav nav-pills round-nav mx-auto" id="myTab1" role="tablist">
					<li class="nav-item font-weight-bolder w-50 m-0">
							...
							<div class="row filesDiv m-0">
								<select id="filesDiv" style="position: relative;" class="image-picker show-html">
									<?php 
										...
										value="{{ $imgKey }}" data-scr="{{ $campaignImg->getFilename() }}" data-lib="campaign_images"> 
											img {{ $imgKey }} 
											<div class="delete-img"><i class="fa fa-trash"></i></div>
										</option>
			...
			<div class="modal-body">
				<div class="card">
					<div class="card-body">
						...
						<h2 class="font-weight-bolder headline-text">{{ $defaultCampaign->headline_text }}</h2>
						<h6 class="text-dark-50 my-4 body-text">
							@php
								echo $defaultCampaign->body_text;
							@endphp
			...
			<form action="#" method="POST" id="updateLoyalClient">
				...
				<div class="modal-body">
					...
					<div class="form-group">
						<label class="font-weight-bold">in the last</label>
						<div class="d-flex align-items-center">
							<input type="text" class="form-control numbers" name="maxMonthsConsideredLoyal" id="maxMonthsConsideredLoyal" value="6" style="border-top-right-radius: 0; border-bottom-right-radius: 0">
							<span class="" style="padding: 0.65rem 1rem; font-size: 16px; font-weight: 400; line-height: 1.50; color: #878c93; background-color: #ffffff; background-clip: padding-box; border: 1px solid #E4E6EF; border-radius: 0 0.42rem 0.42rem 0; border-left: 0;">Months</span>
						</div>
						 {{-- <span class="input-group-text">.00</span> --}}
					</div>
...
<script>
	...
	var element = $("#smartCampaignPreview");
	var getCanvas;
